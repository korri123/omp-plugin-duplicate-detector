...
// ../jscpd/node_modules/.bun/eventemitter3@5.0.1/node_modules/eventemitter3/index.js
...
// ../jscpd/node_modules/.bun/spark-md5@3.0.2/node_modules/spark-md5/spark-md5.js
...
class DuplicateLedger {
  #seen = new Map;
  filterFreshClones(filePath, clones) {
  ...
  formatReminder(clones, filePath, targetFileContent) {
    if (clones.length === 0)
      return "";
...
// ../jscpd/packages/core/src/validators/lines-length-clone.validator.ts
class LinesLengthCloneValidator {
  validate(clone2, options2) {
    const lines = clone2.duplicationA.end.line - clone2.duplicationA.start.line;
    const status = lines >= Number(options2?.minLines);
    return {
      status,
      message: status ? ["ok"] : [`Lines of code less than limit (${lines} < ${options2.minLines})`]
    };
  }
}
// ../jscpd/packages/core/src/validators/validator.ts
function runCloneValidators(clone2, options2, validators) {
  return validators.reduce((acc, validator) => {
    const res = validator.validate(clone2, options2);
    return {
      ...acc,
      status: res.status && acc.status,
      message: res.message ? [...acc.message, ...res.message] : acc.message
    };
  }, { status: true, message: [], clone: clone2 });
}
// ../jscpd/packages/core/src/rabin-karp.ts
class RabinKarp {
  options;
  eventEmitter;
  cloneValidators;
  constructor(options2, eventEmitter, cloneValidators) {
    this.options = options2;
    this.eventEmitter = eventEmitter;
    this.cloneValidators = cloneValidators;
  }
  async run(tokenMap, store2) {
    return new Promise((resolve) => {
      ...
      const loop = () => {
        ...
        store2.get(iteration.value.id).then((mapFrameFromStore) => {
          mapFrameInStore = mapFrameFromStore;
          if (!clone2) {
            clone2 = RabinKarp.createClone(tokenMap.getFormat(), iteration.value, mapFrameInStore);
          }
        }, () => {
          if (clone2 && this.validate(clone2)) {
            clones.push(clone2);
          }
          clone2 = null;
          if (iteration.value.id) {
            return store2.set(iteration.value.id, iteration.value);
          }
        }).finally(() => {
          if (!iteration.done) {
            if (clone2) {
              clone2 = RabinKarp.enlargeClone(clone2, iteration.value, mapFrameInStore);
            }
            loop();
          } else {
            if (clone2 && this.validate(clone2)) {
  ...
  validate(clone2) {
    const validation = runCloneValidators(clone2, this.options, this.cloneValidators);
    if (validation.status) {
      this.eventEmitter.emit("CLONE_FOUND", { clone: clone2 });
    } else {
      this.eventEmitter.emit("CLONE_SKIPPED", { clone: clone2, validation });
    }
  ...
  static enlargeClone(clone2, mapFrameA, mapFrameB) {
    clone2.duplicationA.range[1] = mapFrameA.end.range[1];
    clone2.duplicationA.end = mapFrameA?.end?.loc?.end;
    clone2.duplicationB.range[1] = mapFrameB.end.range[1];
    clone2.duplicationB.end = mapFrameB?.end?.loc?.end;
    return clone2;
  }
}

// ../jscpd/packages/core/src/mode.ts
function strict(token2) {
  return token2.type !== "ignore";
}
function mild(token2) {
  return strict(token2) && token2.type !== "empty" && token2.type !== "new_line";
}

// ../jscpd/node_modules/.bun/eventemitter3@5.0.1/node_modules/eventemitter3/index.mjs
var import__ = __toESM(require_eventemitter3(), 1);
var eventemitter3_default = import__.default;

// ../jscpd/packages/core/src/detector.ts
class Detector extends eventemitter3_default {
  tokenizer;
  store;
  cloneValidators;
  options;
  algorithm;
  constructor(tokenizer2, store2, cloneValidators = [], options2) {
    super();
...
// ../jscpd/packages/core/src/store/memory.ts
class MemoryStore {
  _namespace = "";
  values = {};
  namespace(namespace) {
...
// ../jscpd/packages/tokenizer/src/engine/prism-core.ts
var uniqueID = 0;
...
function extend(id, redef) {
  const lang = clone2(languages[id]);
...
function matchGrammar(text, strarr, grammar, index, startPos, oneshot, target) {
  Object.keys(grammar).forEach((token2) => {
    if (!grammar[token2])
      return;
    if (token2 === target)
      return;
    let patterns = grammar[token2];
    ...
    patterns.forEach((pattern) => {
      ...
      for (let i = index, pos = startPos;i < strarr.length; pos += strarr[i].length, i++) {
        ...
        const wrapped = new Token(token2, inside ? tokenize(matchStr, inside) : matchStr, alias, matchStr, greedy);
        args.push(wrapped);
        if (after)
          args.push(after);
        Array.prototype.splice.apply(strarr, args);
        if (delNum !== 1)
          matchGrammar(text, strarr, grammar, i, pos, true, token2);
        if (oneshot)
          break;
      }
    });
...
function tokenize(text, grammar) {
  ...
  if (rest) {
    Object.keys(rest).forEach((token2) => {
...
var Prism = {
  ...
  tokenize,
  hooks,
  util: { type: getType, objId, clone: clone2 },
  Token
};
var prism_core_default = Prism;

// ../jscpd/packages/tokenizer/src/languages/markup.ts
var markup_default = {
  language: "markup",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/clike.ts
var clike_default = {
  language: "clike",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/css.ts
var css_default = {
  language: "css",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/c.ts
var c_default = {
  language: "c",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/javascript.ts
...
// ../jscpd/packages/tokenizer/src/languages/java.ts
var java_default = {
  language: "java",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/ruby.ts
var ruby_default = {
  language: "ruby",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/php.ts
var php_default = {
  language: "php",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/markup-templating.ts
          ...
          var walkTokens = function(tokens) {
            ...
            for (let i = 0;i < tokens.length; i++) {
              ...
              if (typeof token2 === "string" || token2.content && typeof token2.content === "string") {
                ...
                const s = typeof token2 === "string" ? token2 : token2.content;
              ...
              } else if (token2.content && typeof token2.content !== "string") {
...
// ../jscpd/packages/tokenizer/src/languages/css-extras.ts
...
// ../jscpd/packages/tokenizer/src/languages/scss.ts
var scss_default = {
  language: "scss",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/sass.ts
var sass_default = {
  language: "sass",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/less.ts
var less_default = {
  language: "less",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/cpp.ts
var cpp_default = {
  language: "cpp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/bison.ts
var bison_default = {
  language: "bison",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/objectivec.ts
...
// ../jscpd/packages/tokenizer/src/languages/scala.ts
var scala_default = {
  language: "scala",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/csharp.ts
var csharp_default = {
  language: "csharp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/dart.ts
var dart_default = {
  language: "dart",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/d.ts
var d_default = {
  language: "d",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/fsharp.ts
var fsharp_default = {
  language: "fsharp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/glsl.ts
var glsl_default = {
  language: "glsl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/go.ts
var go_default = {
  language: "go",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/groovy.ts
var groovy_default = {
  language: "groovy",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/haxe.ts
var haxe_default = {
  language: "haxe",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/jolie.ts
var jolie_default = {
  language: "jolie",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/kotlin.ts
var kotlin_default = {
  language: "kotlin",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/reason.ts
var reason_default = {
  language: "reason",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/swift.ts
var swift_default = {
  language: "swift",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/crystal.ts
var crystal_default = {
  language: "crystal",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/erb.ts
var erb_default = {
  language: "erb",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/actionscript.ts
...
// ../jscpd/packages/tokenizer/src/languages/coffeescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/flow.ts
var flow_default = {
  language: "flow",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/n4js.ts
var n4js_default = {
  language: "n4js",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/typescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/jsx.ts
var jsx_default = {
  language: "jsx",
  init: (Prism2) => {
      ...
      for (let i = 0;i < tokens.length; i++) {
        ...
        if (typeof token2 !== "string") {
          if (token2.type === "tag" && token2.content[0] && token2.content[0].type === "tag") {
            ...
            } else if (token2.content[token2.content.length - 1].content === "/>") {} else {
              openedTags.push({
...
// ../jscpd/packages/tokenizer/src/languages/tsx.ts
var tsx_default = {
  language: "tsx",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/arduino.ts
var arduino_default = {
  language: "arduino",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/django.ts
var django_default = {
  language: "django",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/aspnet.ts
var aspnet_default = {
  language: "aspnet",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/velocity.ts
...
// ../jscpd/packages/tokenizer/src/languages/parser.ts
var parser_default = {
  language: "parser",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/php-extras.ts
...
// ../jscpd/packages/tokenizer/src/languages/abap.ts
var abap_default = {
  language: "abap",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/ada.ts
var ada_default = {
  language: "ada",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/apacheconf.ts
...
// ../jscpd/packages/tokenizer/src/languages/apl.ts
var apl_default = {
  language: "apl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/applescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/arff.ts
var arff_default = {
  language: "arff",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/asciidoc.ts
...
// ../jscpd/packages/tokenizer/src/languages/asm6502.ts
var asm6502_default = {
  language: "asm6502",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/autohotkey.ts
...
// ../jscpd/packages/tokenizer/src/languages/autoit.ts
var autoit_default = {
  language: "autoit",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/bash.ts
var bash_default = {
  language: "bash",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/basic.ts
var basic_default = {
  language: "basic",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/batch.ts
var batch_default = {
  language: "batch",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/brainfuck.ts
...
// ../jscpd/packages/tokenizer/src/languages/bro.ts
var bro_default = {
  language: "bro",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/clojure.ts
var clojure_default = {
  language: "clojure",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/csp.ts
var csp_default = {
  language: "csp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/diff.ts
var diff_default = {
  language: "diff",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/docker.ts
var docker_default = {
  language: "docker",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/eiffel.ts
var eiffel_default = {
  language: "eiffel",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/elixir.ts
var elixir_default = {
  language: "elixir",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/erlang.ts
var erlang_default = {
  language: "erlang",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/fortran.ts
var fortran_default = {
  language: "fortran",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/gedcom.ts
var gedcom_default = {
  language: "gedcom",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/gherkin.ts
var gherkin_default = {
  language: "gherkin",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/git.ts
var git_default = {
  language: "git",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/graphql.ts
var graphql_default = {
  language: "graphql",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/haml.ts
var haml_default = {
  language: "haml",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/handlebars.ts
...
// ../jscpd/packages/tokenizer/src/languages/haskell.ts
var haskell_default = {
  language: "haskell",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/hpkp.ts
var hpkp_default = {
  language: "hpkp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/hsts.ts
var hsts_default = {
  language: "hsts",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/http.ts
var http_default = {
  language: "http",
  init: (Prism2) => {
    ...
    for (const contentType in httpLanguages) {
      if (httpLanguages[contentType]) {
        ...
        Prism2.languages.insertBefore("http", "header-name", options3);
      }
    }
  }
};

// ../jscpd/packages/tokenizer/src/languages/ichigojam.ts
...
// ../jscpd/packages/tokenizer/src/languages/icon.ts
var icon_default = {
  language: "icon",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/inform7.ts
var inform7_default = {
  language: "inform7",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/ini.ts
var ini_default = {
  language: "ini",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/io.ts
var io_default = {
  language: "io",
  init: (Prism2) => {
    Prism2.languages.io = {
...
// ../jscpd/packages/tokenizer/src/languages/j.ts
var j_default = {
  language: "j",
  init: (Prism2) => {
    Prism2.languages.j = {
...
// ../jscpd/packages/tokenizer/src/languages/json.ts
var json_default = {
  language: "json",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/julia.ts
var julia_default = {
  language: "julia",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/keyman.ts
var keyman_default = {
  language: "keyman",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/latex.ts
var latex_default = {
  language: "latex",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/liquid.ts
var liquid_default = {
  language: "liquid",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/lisp.ts
var lisp_default = {
  language: "lisp",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/livescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/lolcode.ts
var lolcode_default = {
  language: "lolcode",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/lua.ts
var lua_default = {
  language: "lua",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/makefile.ts
...
// ../jscpd/packages/tokenizer/src/languages/markdown-format.ts
...
// ../jscpd/packages/tokenizer/src/languages/matlab.ts
var matlab_default = {
  language: "matlab",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/mel.ts
var mel_default = {
  language: "mel",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/mizar.ts
var mizar_default = {
  language: "mizar",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/monkey.ts
var monkey_default = {
  language: "monkey",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/nasm.ts
var nasm_default = {
  language: "nasm",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/nginx.ts
var nginx_default = {
  language: "nginx",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/nim.ts
var nim_default = {
  language: "nim",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/nix.ts
var nix_default = {
  language: "nix",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/nsis.ts
var nsis_default = {
  language: "nsis",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/ocaml.ts
var ocaml_default = {
  language: "ocaml",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/opencl.ts
var opencl_default = {
  language: "opencl",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/oz.ts
var oz_default = {
  language: "oz",
  init: (Prism2) => {
    Prism2.languages.oz = {
...
// ../jscpd/packages/tokenizer/src/languages/parigp.ts
var parigp_default = {
  language: "parigp",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/pascal.ts
var pascal_default = {
  language: "pascal",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/perl.ts
var perl_default = {
  language: "perl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/plsql.ts
var plsql_default = {
  language: "plsql",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/powershell.ts
...
// ../jscpd/packages/tokenizer/src/languages/processing.ts
...
// ../jscpd/packages/tokenizer/src/languages/prolog.ts
var prolog_default = {
  language: "prolog",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/properties.ts
...
// ../jscpd/packages/tokenizer/src/languages/protobuf.ts
...
// ../jscpd/packages/tokenizer/src/languages/pug.ts
var pug_default = {
  language: "pug",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/puppet.ts
var puppet_default = {
  language: "puppet",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/pure.ts
var pure_default = {
  language: "pure",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/python.ts
var python_default = {
  language: "python",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/q.ts
var q_default = {
  language: "q",
  init: (Prism2) => {
    Prism2.languages.q = {
...
// ../jscpd/packages/tokenizer/src/languages/qore.ts
var qore_default = {
  language: "qore",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/r.ts
var r_default = {
  language: "r",
  init: (Prism2) => {
    Prism2.languages.r = {
...
// ../jscpd/packages/tokenizer/src/languages/renpy.ts
var renpy_default = {
  language: "renpy",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/rest.ts
var rest_default = {
  language: "rest",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/rip.ts
var rip_default = {
  language: "rip",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/roboconf.ts
...
// ../jscpd/packages/tokenizer/src/languages/rust.ts
var rust_default = {
  language: "rust",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/sas.ts
var sas_default = {
  language: "sas",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/scheme.ts
var scheme_default = {
  language: "scheme",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/smalltalk.ts
...
// ../jscpd/packages/tokenizer/src/languages/smarty.ts
var smarty_default = {
  language: "smarty",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/soy.ts
var soy_default = {
  language: "soy",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/stylus.ts
var stylus_default = {
  language: "stylus",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/tcl.ts
var tcl_default = {
  language: "tcl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/textile.ts
var textile_default = {
  language: "textile",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/twig.ts
var twig_default = {
  language: "twig",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/vbnet.ts
var vbnet_default = {
  language: "vbnet",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/verilog.ts
var verilog_default = {
  language: "verilog",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/vhdl.ts
var vhdl_default = {
  language: "vhdl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/vim.ts
var vim_default = {
  language: "vim",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/visual-basic.ts
...
// ../jscpd/packages/tokenizer/src/languages/wasm.ts
var wasm_default = {
  language: "wasm",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/wiki.ts
var wiki_default = {
  language: "wiki",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/xeora.ts
var xeora_default = {
  language: "xeora",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/xojo.ts
var xojo_default = {
  language: "xojo",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/yaml.ts
var yaml_default = {
  language: "yaml",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/sql.ts
var sql_default = {
  language: "sql",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/antlr4.ts
var antlr4_default = {
  language: "antlr4",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/apex.ts
var apex_default = {
  language: "apex",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/awk.ts
var awk_default = {
  language: "awk",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/bicep.ts
var bicep_default = {
  language: "bicep",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/cfscript.ts
...
// ../jscpd/packages/tokenizer/src/languages/cmake.ts
var cmake_default = {
  language: "cmake",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/csv.ts
var csv_default = {
  language: "csv",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/dot.ts
var dot_default = {
  language: "dot",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/elm.ts
var elm_default = {
  language: "elm",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/gdscript.ts
...
// ../jscpd/packages/tokenizer/src/languages/gettext.ts
var gettext_default = {
  language: "gettext",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/hcl.ts
var hcl_default = {
  language: "hcl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/idris.ts
var idris_default = {
  language: "idris",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/ignore.ts
var ignore_default = {
  language: "ignore",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/json5.ts
var json5_default = {
  language: "json5",
  init: (Prism2) => {
    var string = /("|')(?:\\(?:\r\n?|\n|.)|(?!\1)[^\\\r\n])*\1/;
...
// ../jscpd/packages/tokenizer/src/languages/lilypond.ts
...
// ../jscpd/packages/tokenizer/src/languages/linker-script.ts
...
// ../jscpd/packages/tokenizer/src/languages/llvm.ts
var llvm_default = {
  language: "llvm",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/log.ts
var log_default = {
  language: "log",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/openqasm.ts
...
// ../jscpd/packages/tokenizer/src/languages/plant-uml.ts
...
// ../jscpd/packages/tokenizer/src/languages/powerquery.ts
...
// ../jscpd/packages/tokenizer/src/languages/purescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/qsharp.ts
var qsharp_default = {
  language: "qsharp",
  init: (Prism2) => {
    (function(Prism3) {
...
// ../jscpd/packages/tokenizer/src/languages/racket.ts
var racket_default = {
  language: "racket",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/regex.ts
var regex_default = {
  language: "regex",
  init: (Prism2) => {
    var specialEscape = {
...
// ../jscpd/packages/tokenizer/src/languages/rescript.ts
...
// ../jscpd/packages/tokenizer/src/languages/robotframework.ts
...
// ../jscpd/packages/tokenizer/src/languages/solidity.ts
...
// ../jscpd/packages/tokenizer/src/languages/sparql.ts
var sparql_default = {
  language: "sparql",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/tap.ts
var tap_default = {
  language: "tap",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/toml.ts
var toml_default = {
  language: "toml",
  init: (Prism2) => {
    var key = /(?:[\w-]+|'[^'\n\r]*'|"(?:\\.|[^\\"\r\n])*")/.source;
...
// ../jscpd/packages/tokenizer/src/languages/tt2.ts
var tt2_default = {
  language: "tt2",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/turtle.ts
var turtle_default = {
  language: "turtle",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/unrealscript.ts
...
// ../jscpd/packages/tokenizer/src/languages/wgsl.ts
var wgsl_default = {
  language: "wgsl",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/wolfram.ts
var wolfram_default = {
  language: "wolfram",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/xquery.ts
var xquery_default = {
  language: "xquery",
  init: (Prism2) => {
        ...
        if (typeof token2 !== "string") {
          if (token2.type === "tag" && token2.content[0] && token2.content[0].type === "tag") {
            if (token2.content[0].content[0].content === "</") {
              ...
              if (token2.content[token2.content.length - 1].content === "/>") {} else {
                openedTags.push({
                  tagName: stringifyToken(token2.content[0].content[1]),
                  openedBraces: 0
                });
          ...
          } else if (openedTags.length > 0 && token2.type === "punctuation" && token2.content === "{" && (!tokens[i + 1] || tokens[i + 1].type !== "punctuation" || tokens[i + 1].content !== "{") && (!tokens[i - 1] || tokens[i - 1].type !== "plain-text" || tokens[i - 1].content !== "{")) {
            openedTags[openedTags.length - 1].openedBraces++;
          } else if (openedTags.length > 0 && openedTags[openedTags.length - 1].openedBraces > 0 && token2.type === "punctuation" && token2.content === "}") {
            openedTags[openedTags.length - 1].openedBraces--;
...
// ../jscpd/packages/tokenizer/src/languages/zig.ts
var zig_default = {
  language: "zig",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/txt.ts
var txt_default = {
  language: "txt",
  init: (Prism2) => {
...
// ../jscpd/packages/tokenizer/src/languages/index.ts
...
// ../jscpd/packages/tokenizer/src/grammar-loader.ts
...
// ../jscpd/packages/tokenizer/src/formats.ts
...
// ../jscpd/packages/tokenizer/src/hash.ts
var import_spark_md5 = __toESM(require_spark_md5(), 1);
function hash(value) {
  return import_spark_md5.default.hash(value);
}

// ../jscpd/packages/tokenizer/src/token-map.ts
var TOKEN_HASH_LENGTH = 20;
function createTokenHash(token2, hashFn) {
  return hashFn(token2.type + token2.value).substring(0, TOKEN_HASH_LENGTH);
}
function groupByFormat(tokens) {
  const result = {};
  for (const token2 of tokens) {
    if (result[token2.format]) {
      result[token2.format].push(token2);
    } else {
      result[token2.format] = [token2];
    }
  }
  return result;
}
...
class TokensMap {
  ...
  hashMap;
  hashFn;
  constructor(id, data, tokens, format, options3) {
    this.id = id;
    this.data = data;
    this.tokens = tokens;
    this.format = format;
    this.options = options3;
    this.hashFn = options3.hashFunction ?? hash;
    this.hashMap = this.tokens.map((token2) => {
      if (options3.ignoreCase) {
        token2.value = token2.value.toLocaleLowerCase();
      }
      return createTokenHash(token2, this.hashFn);
    }).join("");
  }
  getTokensCount() {
...
function generateMapsForFormats(id, data, tokens, options3) {
  return Object.values(groupByFormat(tokens)).map((formatTokens) => new TokensMap(id, data, formatTokens, formatTokens[0].format, options3));
}
function createTokensMaps(id, data, tokens, options3) {
  return generateMapsForFormats(id, data, tokens, options3);
}

// ../jscpd/packages/tokenizer/src/languages/vue.ts
...
function tokenizeVue(source, id, options3) {
  const { ignoreCase } = options3;
  const allTokens = [];
...
`, contentAbsStart - 1);
      ...
      for (const token2 of blockTokens) {
        if (token2.loc) {
          if (token2.loc.start.line === 1) {
            token2.loc.start.column = (token2.loc.start.column ?? 1) + colOffset;
          }
          if (token2.loc.end.line === 1) {
            token2.loc.end.column = (token2.loc.end.column ?? 1) + colOffset;
          }
          token2.loc.start.line += lineOffset;
          token2.loc.end.line += lineOffset;
        }
        allTokens.push(token2);
      }
    } catch (_e) {}
  }
  allTokens.forEach((token2, idx) => {
    if (token2.loc) {
      token2.loc.start.position = idx;
      token2.loc.end.position = idx;
    }
  });
  let processedTokens = options3.mode ? allTokens.filter((token2) => options3.mode(token2, options3)) : allTokens;
  if (ignoreCase) {
    processedTokens = processedTokens.map((token2) => {
      token2.value = token2.value.toLocaleLowerCase();
      return token2;
    });
  }
  return createTokensMaps(id, source, processedTokens, options3);
}

// ../jscpd/packages/tokenizer/src/languages/astro.ts
...
function tokenizeAstro(source, id, options3) {
...
`.repeat(countNewlines2(innerContent)) + closeTag;
  ...
  const templateTokens = tokenize2(templateSanitized, "markup").filter((token2) => token2.format === "markup" && token2.length > 0);
  for (const token2 of templateTokens) {
    allTokens.push(token2);
  }
  allTokens.forEach((token2, idx) => {
    if (token2.loc) {
  ...
  return createTokensMaps(id, normalized, processedTokens, options3);
}

// ../jscpd/packages/tokenizer/src/languages/svelte.ts
...
function tokenizeSvelte(source, id, options3) {
...
`);
  ...
  const templateTokens = tokenize2(sanitizedSource, "markup").filter((token2) => token2.format === "markup" && token2.length > 0);
  for (const token2 of templateTokens) {
    allTokens.push(token2);
  }
...
`, block.contentStart - 1);
    ...
    const blockTokens = tokenize2(block.blockContent, resolvedFormat).filter((token2) => token2.length > 0);
    for (const token2 of blockTokens) {
      if (token2.loc) {
        if (token2.loc.start.line === 1) {
          token2.loc.start.column = (token2.loc.start.column ?? 1) + colOffset;
...
// ../jscpd/packages/tokenizer/src/languages/markdown-tokenizer.ts
function tokenizeWithPrism(code, language) {
  ...
  let line = 1;
  let column = 1;
  function createTokenFromString(token2, lang) {
    return [{ format: lang, type: "default", value: token2, length: token2.length }];
  }
  function createTokenFromFlatToken(token2, lang) {
    return [{ format: lang, type: token2.type, value: token2.content, length: token2.length }];
  }
  function createTokens(token2, lang) {
    if (typeof token2 === "string")
      return createTokenFromString(token2, lang);
    if (token2.content && typeof token2.content === "string")
      return createTokenFromFlatToken(token2, lang);
    if (token2.content && Array.isArray(token2.content)) {
      const res = [];
      const rawAlias = token2.alias ? token2.alias.replace("language-", "") : null;
      const childLang = rawAlias && rawAlias in FORMATS ? rawAlias : lang;
      for (const t of token2.content)
  ...
  function calcLoc(token2) {
    const val = token2.value;
    let newLines = 0;
    let lastLineLen = 0;
...
`) {
    ...
    token2.loc = { start, end: { line: line + newLines, column, position: length } };
    token2.range = [length, length + token2.length];
  ...
  return tokens.filter((t) => (t.format in FORMATS)).map((token2, index) => calcLoc(token2));
}
...
function tokenizeMarkdown(source, id, options3) {
  const normalized = source.replace(/\r\n/g, `
`);
  const { ignoreCase } = options3;
  const allTokens = [];
...
`.length));
      for (const token2 of tokenizeWithPrism(innerContent, "yaml")) {
        if (token2.length > 0) {
          if (token2.loc) {
            token2.loc.start.line += lineOffset;
            token2.loc.end.line += lineOffset;
...
`);
      ...
      for (const token2 of tokenizeWithPrism(innerContent, resolvedFmt)) {
        if (token2.length > 0) {
          if (token2.loc) {
            token2.loc.start.line += lineOffset;
            token2.loc.end.line += lineOffset;
  ...
  for (const token2 of tokenizeWithPrism(sanitized, "markdown")) {
    if (token2.format === "markdown" && token2.length > 0)
      allTokens.push(token2);
  }
  allTokens.forEach((token2, idx) => {
    if (token2.loc) {
...
// ../jscpd/packages/tokenizer/src/tokenize.ts
var punctuation2 = {
  new_line: /\n/,
  empty: /[ \t\r\n\f\v]+/
};
var patchedLanguages2 = new Set;
...
function tokenize2(code, language) {
  ...
  function calculateLocation(token2, position) {
    const result = token2;
...
`) {
  ...
  function createTokens(token2, lang) {
    ...
    if (token2.content && Array.isArray(token2.content)) {
      const res = [];
      const rawAlias = token2.alias ? sanitizeLangName(token2.alias) : null;
      const childLang = rawAlias && rawAlias in FORMATS ? rawAlias : lang;
      for (const t of token2.content) {
  ...
  return tokens.filter((t) => (t.format in FORMATS)).map((token2, index) => calculateLocation(token2, index));
}
...
function createTokenMapBasedOnCode(id, data, format, options3 = {}) {
  if (format === "vue") {
    return tokenizeVue(data, id, options3);
  }
  if (format === "astro") {
    return tokenizeAstro(data, id, options3);
  }
  if (format === "svelte") {
    return tokenizeSvelte(data, id, options3);
  }
  if (format === "markdown") {
    return tokenizeMarkdown(data, id, options3);
  }
  const { mode: mode2, ignoreCase, ignorePattern } = options3;
  const tokens = tokenize2(data, format).filter((token2) => mode2(token2, options3));
  if (ignorePattern)
    setupIgnorePatterns(format, options3.ignorePattern || []);
  if (ignoreCase) {
    return createTokensMaps(id, data, tokens.map((token2) => {
      token2.value = token2.value.toLocaleLowerCase();
      return token2;
    }), options3);
  }
  return createTokensMaps(id, data, tokens, options3);
}
// ../jscpd/packages/tokenizer/src/index.ts
class Tokenizer {
  generateMaps(id, data, format, options3) {
    return createTokenMapBasedOnCode(id, data, format, options3);
  }
}
...
class ExportableMemoryStore extends MemoryStore {
  getNamespaceValues() {
    return this.values;
  }
...
class JscpdIndexManager {
  ...
  constructor(options3 = {}) {
    ...
    this.#options = {
      minTokens: options3.minTokens ?? 40,
      minLines: options3.minLines ?? 5,
      maxLines: options3.maxLines ?? 500,
      formatsExts: options3.formatsExts
    };
  ...
  async initialize(rootDir, ignorePatterns, signal) {
    if (this.#initPromise) {
  ...
  async#runInitialize(rootDir, ignorePatterns, signal) {
    ...
    for (const filePath of files) {
      ...
      try {
        ...
        for (const clone3 of clones) {
          const id = cloneIdentity(clone3);
          if (!seenCloneIds.has(id)) {
            seenCloneIds.add(id);
            this.#discoveredClones.push(clone3);
          }
        }
      } catch {}
    }
  ...
  async checkSnippet(targetPath, content) {
    if (this.#initPromise) {
    ...
    if (!format)
      return [];
    const relPath = path.isAbsolute(targetPath) && this.#rootDir ? path.relative(this.#rootDir, targetPath) : targetPath;
    ...
    const relevantClones = allClones.filter((clone3) => {
      const isAQuery = clone3.duplicationA.sourceId === virtualId;
      const isBQuery = clone3.duplicationB.sourceId === virtualId;
      if (isAQuery && !isBQuery) {
        if (clone3.duplicationB.sourceId === relPath) {
          return false;
        }
        return true;
      }
  ...
  async updateFile(targetPath, content) {
    if (this.#initPromise) {
      await this.#initPromise;
    }
  ...
  formatReport(clones = this.#discoveredClones, scanPath = this.#rootDir) {
...
`;
    for (let i = 0;i < clones.length; i++) {
      const clone3 = clones[i];
      const a = clone3.duplicationA;
      const b = clone3.duplicationB;
      const linesCount = a.end.line - a.start.line + 1;
      report += `### Clone #${i + 1} (${linesCount} lines, format: ${clone3.format})
`;
...
`;
  ...
  async#collectFiles(dir, baseDir, ignorePatterns, signal) {
    const results = [];
    if (signal?.aborted)
      return results;
    let entries;
...
function resolveConfig(rawSettings) {
  if (!rawSettings)
  ...
  const reminderMode = rawSettings.reminderMode === "steer" || rawSettings.reminderMode === "none" || rawSettings.reminderMode === "in-band" ? rawSettings.reminderMode : DEFAULT_CONFIG.reminderMode;
  let ignorePatterns = [];
  if (typeof rawSettings.ignorePatterns === "string") {
    ignorePatterns = rawSettings.ignorePatterns.split(",").map((p) => p.trim()).filter((p) => p.length > 0);
...
function duplicateDetectorExtension(pi) {
  const z = pi.zod;
  ...
  pi.on("tool_result", async (event, ctx) => {
    if (event.isError)
      return;
    if (!config.checkOnMutation)
      return;
    if (config.reminderMode === "none")
      return;
    ...
    if (rawPath.includes("://"))
      return;
    const fullPath = path2.isAbsolute(rawPath) ? rawPath : path2.join(ctx.cwd, rawPath);
    const relPath = path2.relative(ctx.cwd, fullPath) || rawPath;
    try {
      ...
      if (freshClones.length > 0) {
        ...
        const originalContent = Array.isArray(event.content) ? event.content : [{ type: "text", text: String(event.content ?? "") }];
  ...
  pi.registerTool({
    ...
    description: "Scan the codebase or a target directory for duplicate code blocks, copy-pasted logic, and code clones using jscpd.",
    parameters: z.object({
      path: z.string().optional().describe("Root directory or path to scan (defaults to project workspace root)"),
      minLines: z.number().optional().describe("Minimum number of consecutive matching lines to report (default: 5)"),
      minTokens: z.number().optional().describe("Minimum token count threshold for a clone block (default: 40)")
    }),
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const scanPath = params.path ? path2.isAbsolute(params.path) ? params.path : path2.join(ctx.cwd, params.path) : ctx.cwd;
  ...
  pi.registerCommand("duplicates", {
    ...
    getArgumentCompletions: (prefix) => {
      const options3 = ["--min-lines=5", "--min-tokens=40", "--path="];
      return options3.filter((opt) => opt.startsWith(prefix)).map((label) => ({ value: label, label, description: `Option: ${label}` }));
    },
    handler: async (args, ctx) => {
      ...
      for (const part of parts) {
        if (part.startsWith("--min-lines=")) {
          ...
          if (!Number.isNaN(val))
            minLines = Math.max(3, val);
        } else if (part.startsWith("--min-tokens=")) {
          const val = Number.parseInt(part.slice("--min-tokens=".length), 10);
          if (!Number.isNaN(val))
            minTokens = Math.max(10, val);
        } else if (part.startsWith("--path=")) {
          targetPath = path2.resolve(ctx.cwd, part.slice("--path=".length));
        } else if (!part.startsWith("-")) {
          targetPath = path2.resolve(ctx.cwd, part);
        }
      }
      try {
        ...
        ctx.ui.notify(`Duplicate scan finished: ${count} files indexed, ${scanEngine.discoveredClones.length} duplicate clusters found.`, scanEngine.discoveredClones.length > 0 ? "warning" : "info");
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
...
export {
  ...
  JscpdIndexManager,
  cloneIdentity,
  duplicateDetectorExtension as default,
  extractLineRange,
  resolveConfig
};
